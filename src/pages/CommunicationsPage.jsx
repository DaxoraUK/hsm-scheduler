...
export default function CommunicationsPage(props) {
  ...
  const approvalByDay = useMemo(() => ({
    saturday: { dayScope: "saturday", matchdayDate: props.satDate || props.satDateLabel, snapshotHash: buildMatchdaySnapshotHash(props.satFinal) },
    sunday: { dayScope: "sunday", matchdayDate: props.sunDate || props.sunDateLabel, snapshotHash: buildMatchdaySnapshotHash(props.sunFinal) },
    midweek: { dayScope: "midweek", matchdayDate: props.midweekDate || props.midweekDateLabel, snapshotHash: buildMatchdaySnapshotHash(props.midweekFinal) },
  }), [props.midweekDate, props.midweekDateLabel, props.midweekFinal, props.satDate, props.satDateLabel, props.satFinal, props.sunDate, props.sunDateLabel, props.sunFinal]);

  const approvalsForRows = (selectedRows = []) => [...new Set(selectedRows.map((row) => row.day).filter(Boolean))]
    .map((dayKey) => approvalByDay[dayKey])
    .filter((approval) => approval?.matchdayDate && approval?.snapshotHash);

  const assertCurrentMatchdayApproval = async (selectedRows = []) => {
    const approvals = approvalsForRows(selectedRows);
    if (!approvals.length) return true;
    try {
      await Promise.all(approvals.map((approval) => DB.assertMatchdayApproval(props.activeClubId, approval)));
      return true;
    } catch (error) {
  ...
  const copyMessage = async (row) => {
    ...
    if (!(await assertCurrentMatchdayApproval([row]))) return;
    try {
  ...
  const copySelected = async (selectedRows) => {
    ...
    if (!(await assertCurrentMatchdayApproval(selectedRows))) return;
    try {
  ...
  const confirmCoachHubPublish = async () => {
    ...
    try {
      ...
      const results = await Promise.all(Object.entries(groups).map(([dayKey, dayRows]) => DB.publishCoachHubMatchweekMessages(props.activeClubId, dayRows.map((row) => ({
        ...
        })), approvalByDay[dayKey])));
      const result = results.reduce((total, item) => ({ published: total.published + Number(item?.published || 0), reused: total.reused + Number(item?.reused || 0) }), { published: 0, reused: 0 });
  ...
  const confirmWebSend = async () => {
    ...
    try {
      ...
      const result = await dispatchCommunicationBatch({
        ...
        requestKey,
        matchdayApprovals: approvalsForRows(confirmation.rows),
      });
      await loadEvents();
