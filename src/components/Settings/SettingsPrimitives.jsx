    ...
    <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex min-w-0 items-start gap-3.5">
        ...
        <div className="min-w-0">
          ...
          {description ? <p className="mt-1.5 max-w-3xl text-sm font-semibold leading-6 text-slate-500">{description}</p> : null}
        </div>
      </div>
...
export function Field({ label, hint, children, className = "" }) {
  return (
    <label className={`block ${className}`}>
      ...
      {hint ? <span className="mt-2 block text-xs font-semibold leading-5 text-slate-400">{hint}</span> : null}
    </label>
  );
}
...
export function StatTile({ label, value, detail, tone = "slate" }) {
  ...
  return (
    <div className={`rounded-[22px] border p-4 ${tones[tone] || tones.slate}`}>
      ...
      {detail ? <div className="mt-1 text-xs font-bold opacity-65">{detail}</div> : null}
    </div>
  );
}
...
export function Toggle({ checked, onChange, label, description, disabled = false }) {
  return (
    <div className={`flex items-center justify-between gap-5 rounded-[22px] border border-slate-200 p-4 ${disabled ? "bg-slate-50 opacity-60" : "bg-white"}`}>
      <div>
        <div className="text-sm font-black text-slate-950">{label}</div>
        {description ? <div className="mt-1 text-sm font-semibold leading-5 text-slate-500">{description}</div> : null}
      </div>
      <button
        type="button"
        role="switch"
...
export function Notice({ children, tone = "info" }) {
  ...
  return (
    <div className={`flex items-start gap-3 rounded-[22px] border p-4 text-sm font-semibold leading-6 ${tones[tone] || tones.info}`}>
