...
export default function CoachRequestConversation({ clubId, request, role = "coach", onClose, embedded = false }) {
  ...
  const load = useCallback(async ({ quiet = false } = {}) => {
    ...
    try {
      ...
      setEntries(Array.isArray(payload?.messages) ? payload.messages : []);
      setStatus("ready");
    } catch (error) {
      setStatus("error");
    ...
    <section className={`${embedded ? "rounded-2xl border border-slate-200" : "max-h-[92vh] w-full max-w-2xl rounded-[28px]"} flex min-h-[420px] flex-col overflow-hidden bg-white shadow-2xl`}>
      <header className="flex items-start gap-3 border-b border-slate-200 p-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-xs font-semibold text-slate-500">{request?.teamName || request?.team_name || "Team"} · messages stay attached to this request</p>
        </div>
