  ...
  return (
    <article className={`rounded-2xl border p-4 ${calendarEventTone(event)}`}>
      ...
      <div className="mt-3 space-y-1.5 text-xs font-bold opacity-80">
        ...
        <div className="flex items-center gap-2"><MapPin size={14} /> {[event.venueName, event.pitchName].filter(Boolean).join(" · ") || (unavailable ? "Club facilities" : "Venue TBC")}</div>
      </div>
