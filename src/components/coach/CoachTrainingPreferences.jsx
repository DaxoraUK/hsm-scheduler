  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        ...
        <label><span className="mb-2 block text-xs font-black text-slate-700">Preferred start times</span><input disabled={readOnly} className="input" value={form.preferredStartTimes.join(", ")} onChange={(event) => set("preferredStartTimes", event.target.value.split(/[,;]+/).map((value) => value.trim()).filter(Boolean))} placeholder="18:00, 19:00" /></label>
