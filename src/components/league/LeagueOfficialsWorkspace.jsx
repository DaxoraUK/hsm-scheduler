...
function Postponements({ leagueId, workspace, operations, canEdit, onRefresh, fixtures }) {
  ...
  const selectedFixture = fixtures.find((row) => `${row.targetType}:${row.targetId}` === targetKey);
  return <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]"><Panel className="p-5"><div className="text-xl font-black text-slate-950">Log postponement</div><p className="mt-1 text-sm font-semibold leading-6 text-slate-500">Start a controlled rearrangement record without losing the original fixture details.</p><div className="mt-4 space-y-3"><Field label="Fixture"><select className={INPUT} value={targetKey === ":" ? "" : targetKey} onChange={(event) => { const [targetType, targetId] = event.target.value.split(":"); setForm((current) => ({ ...current, targetType: targetType || "", targetId: targetId || "" })); }}><option value="">Select fixture</option>{fixtures.filter((row) => row.date).map((fixture) => <option key={`${fixture.targetType}:${fixture.targetId}`} value={`${fixture.targetType}:${fixture.targetId}`}>{fixture.date} · {fixture.homeTeamName} v {fixture.awayTeamName}</option>)}</select></Field><Field label="Reason category"><select className={INPUT} value={form.reasonCategory} onChange={(event) => setForm((current) => ({ ...current, reasonCategory: event.target.value }))}><option value="weather">Weather</option><option value="venue_unavailable">Venue unavailable</option><option value="cup_clash">Cup clash</option><option value="club_request">Club request</option><option value="officials">Officials</option><option value="other">Other</option></select></Field><Field label="Reason"><textarea className="min-h-20 w-full rounded-xl border border-slate-200 p-3 text-sm font-semibold" value={form.reason} onChange={(event) => setForm((current) => ({ ...current, reason: event.target.value }))} /></Field><Field label="Rearrangement deadline"><input type="date" className={INPUT} value={form.deadlineOn} onChange={(event) => setForm((current) => ({ ...current, deadlineOn: event.target.value }))} /></Field><Field label="Requesting club"><select className={INPUT} value={form.requestedByClubId} onChange={(event) => setForm((current) => ({ ...current, requestedByClubId: event.target.value }))}><option value="">League / not specified</option>{workspace.clubs.map((club) => <option key={club.id} value={club.id}>{club.name}</option>)}</select></Field></div><button type="button" disabled={!canEdit || !selectedFixture || form.reason.trim().length < 2} onClick={async () => { try { await DB.upsertLeaguePostponement(leagueId, { ...form, originalDate: selectedFixture.date, originalKickOff: selectedFixture.kickOff, originalVenueId: selectedFixture.venueId, status: "rearrangement_required" }); await onRefresh(); setForm({ targetType: "", targetId: "", reasonCategory: "weather", reason: "", deadlineOn: "", requestedByClubId: "", notes: "" }); toast.success("Postponement added to the command queue"); } catch (error) { toast.error("Postponement could not be saved", { description: error?.message }); } }} className={`${BUTTON} mt-4 w-full bg-rose-600 text-white`}><CalendarClock size={15} /> Add to rearrangement queue</button></Panel>
  <Panel className="overflow-hidden"><div className="flex items-center justify-between border-b border-slate-200 p-5"><div><div className="text-xl font-black text-slate-950">Rearrangement queue</div><div className="mt-1 text-sm font-semibold text-slate-500">Prioritised fixture changes with deadlines and audit history.</div></div><Pill tone={operations.postponements.length ? "amber" : "green"}>{operations.postponements.length}</Pill></div><div className="divide-y divide-slate-100">{operations.postponements.map((row) => { const fixture = fixtures.find((item) => item.targetType === row.targetType && item.targetId === row.targetId); return <div key={row.id} className="p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-sm font-black text-slate-950">{fixture ? `${fixture.homeTeamName} v ${fixture.awayTeamName}` : "Historic fixture"}</div><div className="mt-1 text-xs font-semibold text-slate-500">Original: {row.originalDate || "Unknown"} · {row.originalKickOff || "TBC"} · {row.reason}</div><div className="mt-2 flex flex-wrap gap-2"><Pill tone="rose">{row.reasonCategory || "postponed"}</Pill>{row.deadlineOn ? <Pill tone="amber">Deadline {row.deadlineOn}</Pill> : null}</div></div><select disabled={!canEdit} className="h-9 rounded-xl border border-slate-200 bg-white px-2 text-xs font-black" value={row.status} onChange={async (event) => { try { await DB.updateLeaguePostponementStatus(leagueId, row.id, event.target.value); await onRefresh(); toast.success("Rearrangement status updated"); } catch (error) { toast.error("Status could not be updated", { description: error?.message }); } }}><option value="requested">Requested</option><option value="approved">Approved</option><option value="rearrangement_required">Needs rearranging</option><option value="proposed">Date proposed</option><option value="rearranged">Rearranged</option><option value="rejected">Rejected</option><option value="closed">Closed</option></select></div></div>; })}{!operations.postponements.length ? <div className="p-10 text-center"><CheckCircle2 className="mx-auto text-emerald-400" size={34} /><div className="mt-3 text-sm font-black text-slate-600">No fixtures in the rearrangement queue</div></div> : null}</div></Panel></div>;
}
