...
export default function LeagueRegistrationsWorkspace({ leagueId, workspace, initialTab = "command", focusToken = 0, onSummaryChange }) {
  ...
  const reviewRegistration = async (registration, decision) => {
    const notes = decision === "approved" ? (window.prompt("Optional approval note:", "") || "") : (window.prompt(decision === "correction_required" ? "Tell the club what must be corrected:" : "Add the decision reason:", "") || "");
  ...
  const reviewTransfer = async (transfer, decision) => {
    const notes = window.prompt(decision === "approved" ? "Optional transfer approval note:" : "Add the transfer decision reason:", "") || "";
    if (decision !== "approved" && !notes.trim()) return;
    setBusy(true);
    try {
  ...
  const reviewDispensation = async (dispensation, decision) => {
    const notes = window.prompt(decision === "approved" ? "Optional dispensation conditions:" : "Add the decision reason:", "") || "";
    if (decision !== "approved" && !notes.trim()) return;
    setBusy(true);
    try {
  ...
  return <div className="space-y-5">
    <Panel className="overflow-hidden">
      ...
      <div className="overflow-x-auto border-t border-white/10 bg-white p-2"><div className="flex min-w-max gap-2">{TABS.map(([key, label, Icon]) => <button key={key} type="button" onClick={() => { if (key !== tab && !confirmLeave()) return; setTab(key); }} className={`flex h-11 items-center gap-2 rounded-xl px-4 text-xs font-black transition ${tab === key ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-50"}`}><Icon size={15} />{label}{key === "applications" && data.summary.pendingRegistrations + data.summary.correctionRequired ? <span className="rounded-full bg-rose-500 px-2 py-0.5 text-[10px] text-white">{data.summary.pendingRegistrations + data.summary.correctionRequired}</span> : null}{key === "transfers" && data.summary.pendingTransfers ? <span className="rounded-full bg-amber-500 px-2 py-0.5 text-[10px] text-slate-950">{data.summary.pendingTransfers}</span> : null}</button>)}</div></div>
    </Panel>
