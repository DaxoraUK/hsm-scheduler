...
export default function LeagueDisciplineWorkspace({ leagueId, workspace, initialTab = "command", focusToken = 0, onSummaryChange }) {
  ...
  const updateCaseStatus = async (nextStatus) => {
    ...
    const note = await daxoraPrompt({
      title: `Update ${selectedCase.caseReference}`,
      description: `Change this case to ${nextStatus.replaceAll("_", " ")} and record the reason in the audit timeline.`,
      label: "Case note",
  ...
  const reviewAppeal = async (appeal, nextStatus) => {
    const reason = await daxoraPrompt({
      title: "Record appeal outcome",
      description: `Set this appeal to ${nextStatus.replaceAll("_", " ")} and add the decision rationale.`,
      label: "Outcome and rationale",
