...
export default function LeagueFinanceWorkspace({ leagueId, workspace, initialTab = "command", focusToken = 0, onSummaryChange }) {
  ...
  return <div className="space-y-5">
    ...
    {tab === "invoices" ? <div className="grid gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(420px,1.1fr)]">
      ...
      <Panel className="p-6">{showInvoiceForm ? <InvoiceForm draft={invoiceDraft} setDraft={setInvoiceDraft} workspace={workspace} chargeTypes={data.chargeTypes} busy={busy} onLineChange={setInvoiceLine} onChargeSelect={selectChargeForLine} onSave={saveInvoice} onCancel={() => setShowInvoiceForm(false)} /> : selectedInvoice ? <InvoiceDetail invoice={selectedInvoice} canManage={canManage} busy={busy} onEdit={() => startInvoice(selectedInvoice)} onStatus={changeInvoiceStatus} paymentDraft={paymentDraft} setPaymentDraft={setPaymentDraft} onPayment={recordPayment} creditDraft={creditDraft} setCreditDraft={setCreditDraft} onCredit={addCredit} /> : <div className="flex min-h-[380px] items-center justify-center text-center"><div><ReceiptPoundSterling className="mx-auto text-slate-300" size={38} /><div className="mt-3 text-sm font-black text-slate-700">Select or create an invoice</div></div></div>}</Panel>
    </div> : null}
