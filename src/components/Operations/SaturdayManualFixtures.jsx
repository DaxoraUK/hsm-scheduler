...
export default function SaturdayManualFixtures({
  ...
  return (
    <Card
      ...
      action={
        <StatusChip variant={satManual.length ? "info" : "neutral"}>
          {satManual.length ? `${satManual.length} added` : "Optional"}
        </StatusChip>
      }
    >
      <button
      ...
      {showManual && (
        <div className="mt-5 rounded-3xl border border-slate-200 bg-white p-5">
          ...
          <ManualForm
            onAdd={(fixture) => {
              setSatManual((previous) => [...previous, fixture]);
            }}
            cfgList={teamCfg}
            club={club}
          />
