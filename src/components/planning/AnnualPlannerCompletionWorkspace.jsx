...
export default function AnnualPlannerCompletionWorkspace({
  ...
  const publication = useMemo(() => {
    const active = bookings.filter((row) => !["cancelled", "rejected"].includes(row.status));
    const published = active.filter((row) => row.status === "confirmed");
    const awaiting = active.filter((row) => row.status !== "confirmed");
    return { active, published, awaiting, teams: new Set(published.map((row) => row.teamKey || row.teamName).filter(Boolean)).size };
  }, [bookings]);
  ...
  return <div className="space-y-6">
    <section className="rounded-[28px] border border-sky-200 bg-gradient-to-br from-sky-50 to-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-sky-700"><RadioTower size={15} /> Coach publishing</div><h2 className="mt-2 text-2xl font-black text-slate-950">Draft → review → publish</h2><p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-600">Only confirmed bookings appear in Coach Hub and private coach calendar feeds. Publishing or materially changing a booking sends the assigned coach a targeted in-app update.</p></div>
        <div className="grid grid-cols-3 gap-2 sm:min-w-[420px]"><div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-2xl font-black text-slate-950">{publication.awaiting.length}</div><div className="mt-1 text-[10px] font-black uppercase text-amber-700">Awaiting</div></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><div className="text-2xl font-black text-emerald-950">{publication.published.length}</div><div className="mt-1 text-[10px] font-black uppercase text-emerald-700">Published</div></div><div className="rounded-2xl border border-sky-200 bg-sky-50 p-4"><div className="text-2xl font-black text-sky-950">{publication.teams}</div><div className="mt-1 text-[10px] font-black uppercase text-sky-700">Teams</div></div></div>
      </div>
      <div className="mt-5 grid gap-3 md:grid-cols-3"><div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-xs font-black text-slate-950">1 · Draft</div><p className="mt-1 text-xs font-semibold text-slate-500">Requested and provisional entries stay inside club operations.</p></div><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="text-xs font-black text-amber-950">2 · Review</div><p className="mt-1 text-xs font-semibold text-amber-800">Check team, time, pitch, closures and conflicts before confirmation.</p></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><div className="text-xs font-black text-emerald-950">3 · Publish</div><p className="mt-1 text-xs font-semibold text-emerald-800">Confirmed details reach only the assigned coaches and their feeds.</p></div></div>
      {publication.awaiting.length ? <button type="button" disabled={!canManage || saving} onClick={() => setBulk((current) => ({ ...current, commandType: "change_status", status: "confirmed", bookingIds: publication.awaiting.map((row) => row.id) }))} className="mt-4 inline-flex h-11 items-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-black text-white disabled:opacity-40"><CheckCircle2 size={16} /> Prepare {publication.awaiting.length} for publishing</button> : <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-emerald-100 px-4 py-3 text-xs font-black text-emerald-800"><ShieldCheck size={16} /> Every active booking is coach-visible</div>}
    </section>
