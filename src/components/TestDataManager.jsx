...
import { RE, TEST_MIDWEEK, TEST_SAT, TEST_SUN, WH } from "../lib/constants.js";
import { S } from "../lib/styles.js";

const DAY_OPTIONS = [
  { key: "sat", label: "Saturday", fallback: TEST_SAT },
  { key: "sun", label: "Sunday", fallback: TEST_SUN },
  { key: "midweek", label: "Midweek", fallback: TEST_MIDWEEK },
];
    ...
    <div style={S.card} className="np">
      ...
      <div style={S.cb}>
        <div style={{ fontSize: 12, color: "#666", marginBottom: 12 }}>
          Each fixture-day workspace has its own isolated test data. Editing Midweek will never change Saturday or Sunday fixtures.
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: active ? club.primary : "transparent",
        ...
        {list.map((fixture, index) => (
          <div
            key={`${fixture.homeTeam || "fixture"}-${index}`}
            style={{ border: "1px solid #eee", borderRadius: 6, padding: 10, marginBottom: 8, background: "#fafafa" }}
          >
            <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 180px" }}>
                ...
                <select
                  ...
                  onChange={(event) => update(index, "homeTeam", event.target.value)}
                >
              ...
              <div style={{ flex: "1 1 180px" }}>
                ...
                <input
                  ...
                  onChange={(event) => update(index, "awayTeam", event.target.value)}
                  placeholder="e.g. Bolton Wanderers U12"
                />
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" }}>
              ...
              <label style={{ fontSize: 12, display: "flex", alignItems: "center", gap: 5, cursor: "pointer", paddingBottom: 6 }}>
                <input
                  ...
                  onChange={(event) => update(index, "isCup", event.target.checked)}
                />
                Cup game
              </label>
              ...
              <button
                ...
                style={{ marginLeft: "auto", background: "#FEF2F2", color: RE, border: "1px solid #FECACA", borderRadius: 4, padding: "6px 12px", fontSize: 11, cursor: "pointer", fontWeight: 600 }}
              >
                Remove
              </button>
            </div>
        ...
        {list.length === 0 && (
          <div style={{ fontSize: 12, color: "#aaa", textAlign: "center", padding: 16 }}>
            No test fixtures yet. Click Add Fixture to start.
          </div>
        )}

        <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
          <button type="button" onClick={add} style={{ ...S.btn(club.primary) }}>+ Add Fixture</button>
          <button type="button" onClick={save} style={{ ...S.btn(club.secondary) }}>
            {saved === day ? "Saved" : `Save ${activeOption.label} Test Data`}
          </button>
          <button
            type="button"
            onClick={() => setList?.(activeOption.fallback.map((fixture) => ({ ...fixture })))}
            style={{ ...S.btn(RE) }}
          >
            Reset to Demo Data
          </button>
