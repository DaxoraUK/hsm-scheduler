...
export default function PilotIncidentReporter({ clubId = "", page = "", role = "viewer", workspaceName = "Ground Control" }) {
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-[190] flex items-center justify-center bg-slate-950/65 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-labelledby="pilot-incident-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setOpen(false); }}>
      <section className="flex max-h-[calc(100vh-1.5rem)] w-full max-w-xl flex-col overflow-hidden rounded-[28px] bg-white shadow-2xl sm:max-h-[min(760px,calc(100vh-3rem))]">
        <div className="flex shrink-0 items-start justify-between gap-4 bg-slate-950 p-5 text-white sm:p-6"><div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-rose-300"><AlertTriangle size={15} /> Pilot support</div><h2 id="pilot-incident-title" className="mt-2 text-2xl font-black">Report a problem</h2><p className="mt-2 text-sm font-semibold leading-5 text-slate-300">Tell Daxora what went wrong. We will add safe technical details automatically.</p></div><button type="button" aria-label="Close incident report" disabled={busy} onClick={() => setOpen(false)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/10 text-slate-300 hover:bg-white/10"><X size={17} /></button></div>
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5 sm:p-6">
          ...
          <label className="block text-xs font-black text-slate-700">What happened?<textarea value={summary} maxLength={320} onChange={(event) => setSummary(event.target.value)} className="mt-2 min-h-24 w-full resize-y rounded-xl border border-slate-200 p-3 text-sm font-semibold outline-none focus:border-emerald-500" placeholder="Describe the behaviour without adding personal information or fixture details." /></label>
          ...
          <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-xs font-semibold leading-5 text-sky-900">We add the current page, release, screen size and connection state. Passwords, tokens, emails, contacts, teams and fixture records are never included.</div>
        </div>
        <div className="flex shrink-0 justify-end gap-2 border-t border-slate-100 bg-white p-4 sm:px-6"><button type="button" disabled={busy} onClick={() => setOpen(false)} className="h-11 rounded-xl border border-slate-200 px-4 text-sm font-black text-slate-600">Cancel</button><button type="button" disabled={busy || summary.trim().length < 8} onClick={submit} className="inline-flex h-11 items-center gap-2 rounded-xl bg-rose-600 px-4 text-sm font-black text-white disabled:opacity-50">{busy ? <LoaderCircle className="animate-spin" size={17} /> : <Send size={17} />} Submit report</button></div>
      </section>
    </div> : null}
  </>;
}
