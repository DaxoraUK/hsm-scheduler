...
function normaliseRow(row = {}) {
  return {
    ...
    extraHomeRotationOffset: Number(row.extra_home_rotation_offset ?? row.extraHomeRotationOffset ?? 0) === 1 ? 1 : 0,
  ...
  switch (type) {
    ...
    case "division":
      return {
        ...
        max_consecutive_home_away: Math.max(1, Math.min(Number(draft.maxConsecutiveHomeAway ?? draft.max_consecutive_home_away ?? 2) || 2, 6)),
        extra_home_rotation_offset: Number(draft.extraHomeRotationOffset ?? draft.extra_home_rotation_offset ?? 0) === 1 ? 1 : 0,
