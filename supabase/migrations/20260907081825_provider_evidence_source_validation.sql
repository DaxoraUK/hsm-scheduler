...
begin
  ...
  if exists(select 1 from jsonb_array_elements(source_updates) as updates(value)
      where nullif(trim(updates.value->>'id'),'') is null
      or not exists(select 1 from jsonb_array_elements(sources) as configured(value) where configured.value->>'id'=updates.value->>'id'))
