-- Coach-facing calendars are published views: only confirmed bookings appear.
-- Confirmations and material changes generate targeted in-app messages.
begin;
...
begin
  ...
  if tg_op='UPDATE' then
    ...
    if old.status='confirmed' and new.status='confirmed'
      and (old.title,old.team_key,old.team_name,old.opponent_name,old.venue_id,old.venue_name,old.pitch_id,old.pitch_name,old.start_at,old.end_at)
        is not distinct from
          (new.title,new.team_key,new.team_name,new.opponent_name,new.venue_id,new.venue_name,new.pitch_id,new.pitch_name,new.start_at,new.end_at)
...
drop trigger if exists annual_planner_booking_coach_notification on public.annual_planner_bookings;
create trigger annual_planner_booking_coach_notification
after insert or update on public.annual_planner_bookings
for each row execute function private.notify_coach_hub_booking_change();

-- Tighten the signed-in Coach Hub view created in the previous migration.
...
begin
  ...
  return jsonb_build_object(
    'bookings',coalesce((select jsonb_agg(to_jsonb(booking)-'cost_pence'-'supplier_reference'-'admin_notes'||jsonb_build_object('team_key',assignment.team_key,'team_name',assignment.team_name) order by booking.start_at)
      ...
      where booking.club_id=target_club_id and booking.start_at>=start_boundary and booking.start_at<end_boundary and booking.status='confirmed'),'[]'::jsonb),
