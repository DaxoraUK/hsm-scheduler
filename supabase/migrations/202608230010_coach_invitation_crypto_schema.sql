...
-- functions use an empty search path, so cryptographic calls must be explicit.
...
declare actor_id uuid:=auth.uid(); person public.coach_hub_people%rowtype; raw_token text:=encode(extensions.gen_random_bytes(32),'hex'); invitation_id uuid; expires_value timestamptz;
begin
  ...
  values(target_club_id,person.id,lower(person.email),encode(extensions.digest(raw_token,'sha256'),'hex'),actor_id,expires_value)
...
returns jsonb language plpgsql security definer set search_path = '' set row_security = off as $$
declare actor_id uuid:=auth.uid(); person public.coach_hub_people%rowtype; previous_user_id uuid; raw_token text:=encode(extensions.gen_random_bytes(32),'hex'); invitation_id uuid; expires_value timestamptz;
begin
  if actor_id is null or not public.can_manage_club(target_club_id) then raise exception 'Club administrator access required' using errcode='42501'; end if;
  if not private.club_has_entitlement(target_club_id,'annual_planner') then raise exception 'Coach Hub requires the Annual Planner module' using errcode='42501'; end if;
  ...
  update public.coach_hub_invitations set status='revoked',revoked_at=now(),updated_at=now() where club_id=target_club_id and person_id=person.id and status in ('pending','delivery_failed','accepted');
  expires_value:=now()+make_interval(hours=>greatest(24,least(coalesce(expiry_hours,168),720)));
  insert into public.coach_hub_invitations(club_id,person_id,email,token_hash,invited_by,expires_at)
  values(target_club_id,person.id,lower(person.email),encode(extensions.digest(raw_token,'sha256'),'hex'),actor_id,expires_value)
  returning id into invitation_id;
...
revoke all on function public.create_coach_hub_invitation(uuid,uuid,integer), public.reissue_coach_hub_access(uuid,uuid,integer) from public,anon,authenticated;
grant execute on function public.create_coach_hub_invitation(uuid,uuid,integer), public.reissue_coach_hub_access(uuid,uuid,integer) to authenticated;
