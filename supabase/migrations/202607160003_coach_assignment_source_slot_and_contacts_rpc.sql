-- Ground Control v3.10.2.3
-- Repair multi-team coach assignment source values and expose a stable contacts RPC.

alter table public.coach_hub_team_assignments
  drop constraint if exists coach_hub_team_assignments_source_slot_check;

alter table public.coach_hub_team_assignments
  add constraint coach_hub_team_assignments_source_slot_check
  check (source_slot in ('coach','assistant','manual','directory'));
    ...
    select jsonb_agg(jsonb_build_object(
      ...
      'additional_contacts',coalesce((
        ...
        join public.coach_hub_people person
          on person.id=assignment.person_id and person.club_id=assignment.club_id
        where assignment.club_id=contact.club_id
          and assignment.team_key=contact.team_key
          and assignment.status='active'
          and person.status='active'
          and assignment.source_slot in ('manual','directory')
      ),'[]'::jsonb)
...
-- Reassert the legacy grant for installations where a schema-cache refresh hid it.
